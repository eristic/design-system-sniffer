import {
  ...
  cleanup,
  generateComputedStyleReport,
} from "./computedStyleAnalyzer.js";
...
const components = [
  ...
  {
    name: "Link",
    selectors: ["a", '[role="link"]', '[class*="link"]'],
  },
  {
    name: "Text",
    selectors: ["p", "h1", "h2", "h3", "h4", "h5", "h6", '[class*="text"]'],
  },
  {
    name: "Checkbox",
    selectors: ['input[type="checkbox"]', '[class*="checkbox"]'],
  },
  {
    name: "Radio",
  ...
  {
    name: "Tooltip",
    selectors: ['div[role="tooltip"]', '[class*="tooltip"]'],
  },
];
  ...
  try {
    ...
    try {
      ...
      const backupPath = join(
        OLD_COMPUTED_DIR,
        `computed-styles-${timestamp}.json`
      );
    ...
    const computedStyleResults = await analyzeComputedStyles(
      TARGET_URL,
      components
    );
    ...
    const computedStyleReport =
      generateComputedStyleReport(computedStyleResults);
    await writeFile(
      join(OLD_COMPUTED_DIR, "computed-styles.txt"),
      computedStyleReport
    );
