  ...
  useEffect(() => {
    const loadAnalysisData = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to load analysis data: ${response.status} ${response.statusText}`);
        }
  ...
  if (error) {
    return (
      <div className="error-container">
        ...
        <p>{error}</p>
        <p>Please make sure the computed-styles.json file exists in the public directory.</p>
      </div>
    );
  }
